...
def meteor_score(references_corpus, candidate_corpus):
    import nltk
    ...
    scores = [
        nltk.translate.meteor_score.single_meteor_score(
            [str(e) for e in ref], [str(e) for e in pred]
        )
        for ref, pred in zip(references_corpus, candidate_corpus)
    ]
    meteor_score = np.mean(scores)
    return meteor_score
