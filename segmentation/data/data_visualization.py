...
class CocoDataset:
    ...
    def _process_segmentations(self):
        ...
        for idx, segmentation in enumerate(self.coco["annotations"]):
    ...
    def display_image(
        self, image_id, show_bbox=True, show_polys=True, show_crowds=True
    ):
        print("Image")
        print("==================")
        ...
        image = PILImage.open(image_path)  # for png and jpg format images
        print("image Size", image.size)  # for png and jpg format images

        # For dicom images
        # img_name = os.path.join(image_path)
        ...
        for i, seg in enumerate(self.segmentations[image_id]):
            ...
            if seg["iscrowd"] == 0:
                ...
                for seg_points in seg["segmentation"]:
                    ...
                    np.set_printoptions(threshold=np.prod(seg_points.shape))
        ...
        # html += f'<img src="{str(image_path)}" style="position:relative; transform:rotate(90deg); top:0px; left:0px; width:{adjusted_width}px;">'
...
# instances_json_path = "data/annotations/val_coco_style_annot.json"
instances_json_path = "data/annotations/new_CANDIDPTX_val_coco_style_annot.json"
# images_path = "/media/amir_shirian/abd1fa2e-8fdf-46e4-9dbf-dc0a070ba9b6/home/user/Desktop/Segmentation/dataset"
images_path = "data/images/CANDIDPTX"
...
# html = coco_dataset.display_image(36266)
html = coco_dataset.display_image(2943)
